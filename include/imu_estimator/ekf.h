...
public:
    ...
    void predictionUpdate (Eigen::Matrix<double, 7, 1>& predeted_x, Eigen::Matrix<double, 7, 7>& predeted_Cov, const Eigen::Vector3d& gyro) {
    ...
    void observationUpdate (const Eigen::Matrix<double, 7, 1>& predeted_x, const Eigen::Matrix<double, 7, 7>& predeted_Cov,const Eigen::Vector3d& z) {
        Eigen::Vector4d predeted_quat = predeted_x.head<4>();

        /* y = z - h(x) */
        Eigen::Vector3d acc;
        Eigen::Quaternion<double> quat_tmp(predeted_quat[0], predeted_quat[1], predeted_quat[2], predeted_quat[3]);
        acc = quat_tmp.conjugate()._transformVector(gravity_vec_);
        Eigen::Vector3d y = z - acc;

        /* H */
        Eigen::Matrix<double, 3, 7> H;
        double qw = predeted_quat[0], qx = predeted_quat[1], qy = predeted_quat[2], qz = predeted_quat[3];
        H <<
          -qy, +qz, -qw, +qx, 0, 0, 0,
          +qx, +qw, +qz, +qy, 0, 0, 0,
          +qw, -qx, -qy, +qz, 0, 0, 0;
        ...
        /* x_ */
        Eigen::Matrix<double, 7, 3> K = predeted_Cov * H.transpose() * (H * predeted_Cov * H.transpose() + R_).inverse();
    ...
    void filterOneStep (Eigen::Quaternion<double>& quat, const double dt, const Eigen::Vector3d& acc, const Eigen::Vector3d& gyro)
    {
        setdt(dt);
